...
REPO_RAW="$REPO_BASE/projects/gitlab"
...
load_configuration() {
    ...
    ARCANE_ICON="${ARCANE_ICON:-si:gitlab}"
    ARCANE_CATEGORY="${ARCANE_CATEGORY:-DevOps}"
    GITLAB_VERSION="${GITLAB_VERSION:-latest}"
    GITLAB_HTTP_PORT="${GITLAB_HTTP_PORT:-8929}"
    GITLAB_SSH_PORT="${GITLAB_SSH_PORT:-8922}"
}
...
do_uninstall() {
    ...
    UNINSTALL_SVC_NAME="GitLab"
    # shellcheck disable=SC2034
    UNINSTALL_SYSTEMD="container-gitlab.service" 
    # shellcheck disable=SC2034
    UNINSTALL_CONTAINERS=("gitlab")
    ...
    # shellcheck disable=SC2034
    UNINSTALL_IMAGES=("gitlab/gitlab-ce:latest")
    
    # shellcheck disable=SC2034
    UNINSTALL_VOLUMES=()
    # shellcheck disable=SC2034
    UNINSTALL_DIRS=("$INSTALL_DIR/config" "$INSTALL_DIR/logs" "$INSTALL_DIR/data")
    uninstall_generic_service
}

check_existing_installation() {
    local dir="${1:-/opt/gitlab}"
    if [ -f "$dir/.env" ] && podman container exists gitlab 2>/dev/null; then
        return 0
    fi
    return 1
}
...
deploy_and_persist() {
    ...
    verify_containers_running gitlab
    
    rm -f "$INSTALL_DIR"/*.bak 2>/dev/null || true
    
    log "Configuring systemd service for persistence..."
    mkdir -p ~/.config/systemd/user/
    cat <<EOF > ~/.config/systemd/user/container-gitlab.service
[Unit]
Description=GitLab Stack (podman-compose)
...
EOF
    ...
    systemctl --user enable --now container-gitlab.service
}

print_success() {
    echo "================================================================="
    echo " GitLab deployed and secured with systemd."
    echo " URL: http://$HOST_IP:$GITLAB_HTTP_PORT"
    echo " Admin Password: $GITLAB_ROOT_PASSWORD"
    echo "================================================================="
}
...
do_install() {
    ...
    manage_credentials "$INSTALL_DIR" GITLAB_ROOT_PASSWORD
    ...
    generate_runtime_env
    deploy_and_persist
    register_arcane_project "gitlab" "$INSTALL_DIR"
    print_success
}

do_start() {
    systemctl --user start container-gitlab.service
}
...
if check_existing_installation "/opt/gitlab"; then
    if [ -t 0 ] && [ "${FORCE_YES:-0}" -eq 0 ]; then
        echo ""
        echo "================================================================="
        echo " GitLab — Management"
        echo "================================================================="
        echo " Existing installation detected at /opt/gitlab"
        echo ""
