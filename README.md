...
### 1. Podman & Compose Installation

To prepare your server (Debian/Ubuntu/Proxmox), run these direct commands to install Podman and the Compose complement:

```bash
sudo apt update
sudo apt install -y podman podman-compose

# Optional: Install the Docker interface shim (highly recommended)
sudo apt install -y podman-docker
```

### 2. Podman Pod & Network Configuration

To achieve the **"One IP per Project"** architecture, you must configure a persistent internal network. This allows each project (Pod) to have its own isolated identity.

#### Step A: Create the project network

Run this command once to define the subnet for your internal services:

```bash
# Create the bridge network for isolated pods
podman network create --subnet 172.170.1.0/24 internal_net
```

#### Step B: Understand the Pod Hierarchy

In this framework, **one Project = one Pod**. All containers within that project share the same network namespace and the same IP.

**Requested Architecture Example:**

* Project Caddy Proxy (Pod IP: `172.170.1.1`)
  * Container: `caddy` → `172.170.1.1:3000`
  * Container: `caddymanager-ui` → `172.170.1.1:8080`
* Project Anytype (Pod IP: `172.170.1.2`)
  * Container: `any-sync-coordinator` → `172.170.1.2:1000`
  * Container: `any-sync-filenode` → `172.170.1.2:1004`
...
| `podman-compose` | Multi-container stack management (Podman Pod complement) |
