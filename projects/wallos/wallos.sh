...
REPO_RAW="$REPO_BASE/projects/wallos"
...
load_configuration() {
    ...
    INSTALL_DIR="${INSTALL_DIR:-/opt/wallos}"
    # Inicializa tus variables personalizadas aquí
    ARCANE_ICON="${ARCANE_ICON:-material-symbols:account-balance-wallet-outline}"
    ARCANE_CATEGORY="${ARCANE_CATEGORY:-Finance}"
}
...
# Wallos variables
WALLOS_VERSION="$WALLOS_VERSION"
WALLOS_PORT="$WALLOS_PORT"
EOF
    umask "$OLD_UMASK"
}
...
do_uninstall() {
    ...
    UNINSTALL_SVC_NAME="Wallos"
    # shellcheck disable=SC2034
    UNINSTALL_SYSTEMD="container-wallos.service" 
    # shellcheck disable=SC2034
    UNINSTALL_CONTAINERS=("wallos")
    ...
    # shellcheck disable=SC2034
    UNINSTALL_IMAGES=("bellamy/wallos:latest")
    
    # shellcheck disable=SC2034
    UNINSTALL_VOLUMES=("wallos_db" "wallos_logos")
    # shellcheck disable=SC2034
    UNINSTALL_DIRS=()
...
check_existing_installation() {
    local dir="${1:-/opt/wallos}"
    if [ -f "$dir/.env" ] && podman container exists wallos 2>/dev/null; then
        return 0
    fi
    return 1
}
...
deploy_and_persist() {
    ...
    verify_containers_running wallos
    
    rm -f "$INSTALL_DIR"/*.bak 2>/dev/null || true
    
    log "Configuring systemd service for persistence..."
    mkdir -p ~/.config/systemd/user/
    cat <<EOF > ~/.config/systemd/user/container-wallos.service
[Unit]
Description=Wallos Stack (podman-compose)
...
EOF
    ...
    systemctl --user enable --now container-wallos.service
}

print_success() {
    echo "================================================================="
    echo " Wallos deployed and secured with systemd."
    echo " URL: http://$HOST_IP:$WALLOS_PORT"
    echo "================================================================="
}
...
do_install() {
    ...
    generate_runtime_env
    deploy_and_persist
    register_arcane_project "wallos" "$INSTALL_DIR"
    print_success
}

do_start() {
    systemctl --user start container-wallos.service
}
...
if check_existing_installation "/opt/wallos"; then
    if [ -t 0 ] && [ "${FORCE_YES:-0}" -eq 0 ]; then
        echo ""
        echo "================================================================="
        echo " Wallos — Management"
        echo "================================================================="
        echo " Existing installation detected at /opt/wallos"
        echo ""
